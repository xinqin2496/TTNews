...
+ (FMDatabase *)defaultDatabase{
    ...
    dispatch_once(&onceToken, ^{
        //数据库对象初始化，需要数据库路径
    NSString *docPath= [[NSBundle mainBundle]pathForResource:@"data.sqlite" ofType:nil];
...
+(void)videoWithParameters:(TTVideoFetchDataParameter *)videoParameters success:(void (^)(NSArray *array, NSString *maxtime))success failure:(void (^)(NSError *error))failure {
    if ([TTJudgeNetworking currentNetworkingType] == NetworkingTypeNoReachable) {//没有网络
        ...
        [[UIApplication sharedApplication].keyWindow.rootViewController.view showError:@"无网络连接"];
...
+(NSMutableArray *)TTHeaderNewsFromCacheWithMaxTTHeaderNews:(TTHeaderNews *)headerNews {
    ...
    if ([db open]) {
        ...
        int num = arc4random() % 27;
            FMResultSet *result = nil;
                result = [db executeQuery:[NSString stringWithFormat:@"select * from table_ttheadernews order by id desc limit %d,5",num]];
            while (result.next) {
...
//    [_queue inDatabase:^(FMDatabase *db) {
    ...
    if ([db open]) {
        ...
        while (result.next) {
            ...
            news.createdtime  = (NSInteger)[result longLongIntForColumn:@"createdtime"];
