...
-(void)pushToVideoCommentViewControllerWithIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.backVideo = self.videoArray[indexPath.row];
...
- (void)videoplayViewSwitchOrientation:(BOOL)isFull
{
    if (isFull) {
        ...
        [self.fullVc dismissViewControllerAnimated:YES completion:^{
            ...
            self.playView.frame = CGRectMake(0,self.currentSelectedCell.contentLabel.bounds.size.height + 30 + self.currentSelectedCell.headerImageView.bounds.size.height, kScreenW, self.currentSelectedCell.videoImageView.bounds.size.height);
...
-(void)clickVideoButton:(NSIndexPath *)indexPath {
    ...
    TTVideo *video = self.videoArray[indexPath.row];
    playView.frame = CGRectMake(0,cell.contentLabel.bounds.size.height + 30 + cell.headerImageView.bounds.size.height, kScreenW, self.currentSelectedCell.videoImageView.bounds.size.height);
...
{
    NSData *imageData = [NSData dataWithContentsOfURL:[NSURL URLWithString:video.image1]];
    UIImage *image = [UIImage imageWithData:imageData];
    
    ShareView *shareView = [ShareView createViewFromNib];
    
    [shareView setShareController:self Content:video.text Image:image URL:video.profile_image Title:video.screen_name];
