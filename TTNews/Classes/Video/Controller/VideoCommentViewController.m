...
-(void)viewWillDisappear:(BOOL)animated {
    ...
    if (self.saved_top_cmt) {
        self.backVideo.top_cmt = self.saved_top_cmt;
        [self.backVideo setValue:@0 forKeyPath:@"cellHeight"];
    }
    // 取消所有任务
...
{
    // 清空top_cmt
    if (self.backVideo.top_cmt) {
        self.saved_top_cmt = self.backVideo.top_cmt;
        self.backVideo.top_cmt = nil;
        self.backVideo.cellHeight = 0;
    }
    ...
    cell.delegate = self;
    cell.video = self.backVideo;
    
    cell.frame = CGRectMake(0, cellMargin, [UIScreen mainScreen].bounds.size.width, self.backVideo.cellHeight);
    cell.contentView.frame = cell.frame;
    
    // header的高度
    header.height =  self.backVideo.cellHeight + cellMargin;
...
- (void)loadNewComments
{
    ...
    params[@"a"] = @"dataList";
    params[@"c"] = @"comment";
    params[@"data_id"] = self.backVideo.ID;
    params[@"hot"] = @"1";
...
-(void)clickVideoButton:(NSIndexPath *)indexPath {
    ...
    VideoPlayView *playView = [VideoPlayView videoPlayView];
     playView.frame = CGRectMake(0, self.headerVideoCell.headerImageView.bounds.size.height + 30 + self.headerVideoCell.contentLabel.bounds.size.height, kScreenW,self.headerVideoCell.videoImageView.bounds.size.height);
    [self.headerVideoCell addSubview:playView];
    self.headerVideoCell.playView = playView;
    self.playView = playView;
    self.playView.delegate = self;
    AVPlayerItem *item = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:self.backVideo.videouri]];
...
- (void)videoplayViewSwitchOrientation:(BOOL)isFull
{
    if (isFull) {
        ...
        self.playView.frame = CGRectMake(0, self.headerVideoCell.headerImageView.bounds.size.height + 30 + self.headerVideoCell.contentLabel.bounds.size.height, kScreenW,self.headerVideoCell.videoImageView.bounds.size.height);
