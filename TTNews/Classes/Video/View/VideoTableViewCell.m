...
- (void)setVideo:(TTVideo *)video {
    ...
    if (count>10000) {
        self.playCountLabel.text = [NSString stringWithFormat:@"%ld万播放",(long)count/10000];
    } else {
        self.playCountLabel.text = [NSString stringWithFormat:@"%ld播放",(long)count];
    }
    NSInteger time = video.videotime.integerValue;
    self.timelabel.text = [NSString stringWithFormat:@"%02ld%02ld",(long)time/60,(long)time%60];
...
- (IBAction)repost:(id)sender {
    if ([self.delegate respondsToSelector:@selector(clickRepostButton:)]) {
        [self.delegate clickRepostButton:self.video];
    }
}
